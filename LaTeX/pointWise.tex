...
Let $V_0 = \cbr{p_1, p_2, p_3, p_4, p_5}$ be the set of vertices in a plus with a middle point in $\cee$. Then define
\begin{equation}
...
\end{equation}
this gives us a sequence of Vicsek set graphs $(G_m)_{m\geq 0}$ whose vertices are $(V_m)_{m \geq 0}$. Further we will use the notation of $V_{\ast} = \cup_{m \geq 0} V_m$ and for any $p \in V_m$ we denote the collection of neighbours of $p$ in $G_m$ by $V_{m,p}$. Lastly note that $\# V_m = 5^{m}\cd 4 + 1$
...
\end{equation}
\begin{remark*}
	While \cite{strichartz} often calls this form graph energies, it is the Dirichlet form.
\end{remark*}
...
Thus we get our renormalization constant to be $r = \frac{1}{3}$
