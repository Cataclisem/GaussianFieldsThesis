...
\end{equation}
where $\Delta$ is the unique unique densly defined self-adjoint operator which is an extension of the Kigami Laplacian $\Delta_{\mu}$. The operator $\Delta$ with domain $\dom \Delta$ is referred to as the Dirichlet Laplaican.
...
Recalling \cref{discreteMatrix} then we consider the eigenvalues $0 < \lambda_1^{m} \leq \lambda_{2}^{m} \leq \ldots \leq \lambda_{N_m}^{m}$ of $-\Delta_m$. Next let $(\Phi_i^{m})_{1 \leq i \leq N_m}$ be the corresponding eigenfunctions with respect to the measure $\mu_m$. This leads us to defining the following. \textcolor{red}{Still something about eigenfunctions}
...
We can further by construction see that the matrix $(G_s^{m}(x,y))_{x, y \in V_m}$ is symmetric and is non-negative \textcolor{red}{I cannot figure it out, write stuff here later}, thus it can be the covariance matrix of a Gaussian vector.
...
\end{defi}
\begin{remark*}
	The attentive reader will have noticed that $\frac{1}{a_m} \sum_{y \in V_m} \phi_{i}^{m}(y)f(y) = \inn{\phi^{m}_j}{f}_{L^{2}(V_m, \mu_m)}$ and therefore that $(-\Delta_{m})^{-s}f(x) = \sum_{i = 1}^{N_m} (\lambda_{i}^{m})^{-s} \phi_{i}^{m}(x) \inn{\phi_j^{m}}{f}_{L^2(V_m, \mu_m)}$
\end{remark*}
...
\end{prop}
\begin{proof}
	We start with (\ref{DFLuniformConvergent}). We have
	\begin{align}
...
This semigroup admits by \cref{heatKernel} a heat kernel $p_t(x,y), t > 0, x,y \in K$, wrt. the normalized Hausdorff measure $\mu$. The heat kernel $p_t(x,y)$ is called the Dirichlet heat kernel on $K$. Further this heat kernel satisfies that for some $c_1, c_2 \in (0, \infty)$ \textcolor{red}{find ref}
...
\textcolor{red}{The reason that we find eigenfunctions for $-\Delta$ is such that the eigenfunctions satisfy
\begin{equation}
...
\end{defi}
\begin{remark*}
	Ones again we notice that $\inn{\phi_i}{f} = \int_{K} \phi_i f \dd \mu$, and can rewrite $(-\Delta)^{-s} f = \sum_{j = 1}^{\infty} \frac{1}{\lambda_j^{s}} \phi_j \inn{\phi_j}{f}$.
\end{remark*}

Now that we have the smooth fractional Laplacian defined we will show some of its very useful properties.
...
\begin{proof}
	...
	as desired.
\end{proof}
While the definition of the fractional Laplacian is not the most complicated, we are still able to show an equality between $(-\Delta)^-s$ and the Riesz kernel. We are therefore able to use the estimates of the heat kernel $p_t(x,y)$ in conjunction with $(-\Delta)^{-s}$.
\begin{defi}
...
We will now show some of the bounds of the fractional Riesz kernel. 
\begin{prop}
...
\begin{proof}[Proof of \cref{boundsOfRieszKernel}]
	...
	\end{align}
	\textcolor{red}{convergence of the last integral comes from the fact that $e^{-c_2(1/u)^{1/d_w}}$ converges faster than $1/u$ diverges.} Lastly for $s = d_h/d_w$, we again use the change of variable $t = ud(x,y)^{d_w}$ and setting $R = \diam(K)$ and note that $0 < 1/R^{d_w} < 1/d(x,y)^{d_w}$, thus we find
	\begin{align}
		&\phantom{=}\int_{0}^{1} t^{s-1-\frac{d_h}{d_w}} \exp\left(-c_2 \left(\frac{d(x,y)^{d_w}}{t}\right)^{\frac{1}{d_w -1}}\right). \\
...
\end{lemma}
\begin{proof}
	We will first make sure the integral on the left side actually converges. But this holds because $S(K) \subset C(K)$ so $f$ will be bounded on $K$. Thus we just need that for every $x \in K$ $\int_{K}G_s(x,y) \dd \mu(y)$ is convergent. for $s \in [d_h/d_w, \infty)$ the convergence follows from \cref{boundsOfRieszKernel}. Lastly for $s \in (0, d_h/d_w)$ then $d_h-sd_w < d_h$. If we recall \cref{normalziedHausdorffMeasureAhlforsRegular} we can
	\begin{equation}
