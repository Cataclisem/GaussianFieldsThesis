...
\begin{defi}
	...
	\begin{equation}
		\Delta_m f(p) = 3^{m} \sum_{q \in V_{m,p}} (f(q) - f(p)), \quad p \in V_m \backslash V_0
	\end{equation}
\end{defi}
...
The discrete Laplacian will be used to construct the Dirichlet Laplacian which is the one we will be working with in the rest of this thesis. The construction require the construction of the Kigami Laplacian, which we will define as a pointwise limit of the discrete Laplacian. 

\section{We're going to define it again}
In this section we will follow arguments given by \cite{strichartz}. Much of the proof are going to rely on the standard graph Dirichlet form. Thus we are first going to define and prove some properties of this graph Dirichlet form.
...
If one calculates the standard Dirichlet form then you will get
\begin{equation}
...
Thus we get our renormalization constant for the vicsek set to be $r = 3$. And altogether we find that from \cref{renormalizedGraphEnergies} we have that 
\begin{equation}
...
We are now ready to define the Kigami Laplacian. We will denote this with $\Delta$ and we will notice that it depends on a measure $\mu$, which will be the one defined in \eqref{normalziedHausdorffMeasure}.
\begin{defi}
	Let $u \in \dom \mathcal{E}$ and let $f$ be continuos, then $u \in \dom \Delta$ and $\Delta u =f$ if
	\begin{equation}
		\mathcal{E}(u,v) = \int_{K}fv \dd \mu, \quad \forall v \in \dom_{0}\mathcal{E}
	\end{equation}
\end{defi}

As mentioned before  we are going to want $\Delta$ to be defined as the limit of a discrete operator. We are therefore going to again define a discrete graph Laplacian, the difference is that the following will be without constants.
...
\end{defi}
I might seem that we have only defined $\psi_x^{(m)}$ on $V_m$, but it is actually defined on the whole of $K$. It simply follows that we create an harmonic extension of $\psi_x^{(m)}$. We even have that function $\psi_x^{(m)}$ can be used to describe $\widetilde{\Delta}_{m}$.
...
Before we begin the proof $\Delta$ is the limit of $\widetilde{\Delta}_{m}$ we need some preliminary lemmas.
...
\begin{theorem} \label{pointwiseFormula}
	Let $u \in \dom \Delta$. Then the pointwise formula
	\begin{equation}
		\Delta u(x) = \lim_{m \to \infty} r^{-m} \left(\int_{K} \psi_x^{(m)} \dd \mu \right)^{-1} \widetilde{\Delta}_m u(x)
	\end{equation}
	holds with the limit uniform across $V_* \backslash V_0$. Conversely, suppose $u$ is a continuos function and the right side of the limit converges uniformly to a continuos function on $V_* \backslash V_0$. Then $u \in \dom \Delta$ and the equation holds.
\end{theorem}

\begin{proof}
	Suppose $u \in \dom \Delta$ and $\Delta u=f$, then by \cref{pointwiseEqualityofpsi} we have that 
	\begin{equation}
	...
	in which case
	\begin{equation}
		\lim_{m \to \infty} r^{-m} \left(\int_{K} \psi^{(m)}_{x} \dd\mu \right)^{-1} \widetilde{\Delta}_m u(x) = \lim_{m \to \infty} \frac{\int_{K} f \psi_x^{(m)} \dd \mu}{\int_{K} \psi_x^{(m)} \dd \mu} = f(x) = \Delta u(x)
	\end{equation}
	...
	\end{equation}
	and we have that $u \in \text{dom}\Delta$.
...
We will now extend the Kigami Laplacian to the Dirichlet Laplacian, and we will be following \cite{Fabrice} and \cite{specAnal} very closely in defining the Dirichlet Laplacian

This will be done through Dirichlet forms, thus we begin with them. 
\begin{defi}
	\begin{enumerate}[(i)]
		\item A \emph{quadratic form} $\mathcal{E}$ on $\mathcal{H}$ is a non-negative, definite, symmetric bilinear form from $\dom \mathcal{E} \times \dom \mathcal{E} \to \ree$, where $\dom \mathcal{E}$ is a dense subspace $\mathcal{H}$. 
