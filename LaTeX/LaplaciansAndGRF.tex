...
As mentioned before  we are going to want $\Delta_{\mu}$ to be defined as the limit of a discrete operator. We are therefore going to again define a discrete graph Laplacian, the difference is that the following will be without constants.
...
\end{equation}
where $\Delta_{\mu}$ is the unique unique densly defined self-adjoint operator which is an extension of the Kigami Laplacian $\Delta_{mu}$. The operator $\Delta$ with domain $\dom \Delta$ is referred to as the Dirichlet Laplaican.
\textcolor{red}{Something feels wrong, i shoudl use 1.3.5 but i end up using 1.3.3. But then i need that the kigami laplacian is densly defined and self adjoint and so forth.}

\section{Discrete Fractional Gaussian Fields}
Recalling \cref{discreteMatrix} then we consider the eigenvalues $0 < \lambda_1^{m} \leq \lambda_{2}^{m} \leq \ldots \leq \lambda_{N_m}^{m}$ of $-\Delta_m$. Next let $(\Psi_i^{m})_{1 \leq i \leq N_m}$ be the corresponding eigenfunctions with respect to the measure $\mu_m$. This leads us to defining the following. \textcolor{red}{Still something about eigenfunctions}
...
When stating the Fractional Laplacian one use that it admits a heat kernel through a semigroup generated by $-\Delta$. We will therefore first define a semigroup along with showing that $-\Delta$ generates a semigroup and define what the heat kernel is.
...
\begin{proof}
	...
	\end{equation}
	\textcolor{red}{Is enought to show $Q_t$ is all these things since $P_t = Ue^{-\lambda t} U^{-1}$ is still all the things? by def. that unitary maps are surjective.} We will prove that $Q_t = U^{-1}P_tU$ with domain $\dom Q_t$ is a self-adjoint strongly continous contraction semigroup on $L^2(\Omega, \nu)$ with generator $A$.
	
	Starting with generator, we see that
	\begin{align}
	...
	\end{equation}
	Next we have that $\dom Q_t$ is dense in $L^(\Omega, \nu)$ since if $f_n \to f$ in $L^2(\omega, \nu)$ and $Q_t f_n \to g$ then
	\begin{equation}
		\lim_{n \to \infty} Q_t f_n = \lim_{n \to \infty} e^{-\lambda t}f_n = e^{-\lambda t}f = Q_t f.
	\end{equation} 
	
	Now showing that $\dom (Q_{t})^{*} = \dom Q_t$, then since $Q_t$ is symmetric we have that
	\begin{equation}
		\abs{\inn{f}{Q_tg}} = \abs{\inn{Q_t f}{g}} \leq \norm{Q_t f} \norm{g}.
	\end{equation}
	\textcolor{red}{I think this arguments holds. see just below definition of self-adjoint.}
	...
	\end{equation}
	Thus we have that $Q_t$ is a self-adjoint strongly continous contraction semigroup on $L^2(\Omega, \nu)$ with generator $A$.
\end{proof}

With semigroups out of the way we can define the heat kernel.
\begin{defi}\label{heatKernel}
	The heat kernel $p_t(x,y)$ is the function that satisfies
	\begin{equation}
...
We therefore define the fractional Reisz kernel.
\begin{defi}
