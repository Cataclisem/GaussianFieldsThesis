...
We further note that for $s > \frac{d_h}{2d_w}$
\begin{equation}\label{convergenceOfEigenvaluesOfLaplacian}
	\sum_{j = 1}^{\infty} \frac{1}{\lambda_{j}^{2s}} < \infty.
\end{equation}
this follows from \cite[lemma 5.1.3]{Kigami_2001} which states that there exists a constant $c$ such that $\lambda_{n} \leq c\cd n^{2/d_s}$.
...
\begin{defi}
	...
	\end{equation}
\end{defi}
By \cite[Theorem 2]{germanOne} and \eqref{convergenceOfEigenvaluesOfLaplacian} we have that $S(K)$ is a Frèchet Nuclear space. \textcolor{red}{why? $D(T^{\infty}) = \cap_{n \geq 0} D(T^{n})$ which doesn't look like our space}.
...
\begin{proof}
	...
	\begin{equation}
		\int_{K}\phi_j \phi_i \dd \mu = \begin{cases}
			0, & \text{if } i \neq j \\
			1, & \text{if } i = j
		\end{cases}
	\end{equation}
	so for any eigenfunction $\phi_n$ we have
	\begin{equation}
		(-\Delta)^{-s}\phi_n = \sum_{j = 1}^{\infty} \frac{1}{\lambda_{j}^{s}} \phi_j(x) \int_{K}\phi_j(y)\phi_n \dd \mu(y) = \frac{1}{\lambda_{n}^{-s}}\phi_{n}.
	\end{equation}
	Now from the definition of $(-\Delta)^{-s}$ and the previous lemma, we find that
	\begin{align}
		\int_{K} (-\Delta)^{-s}f (-\Delta)^{-s}g \dd \mu &= \inn{(-\Delta)^{-s}f}{(-\Delta)^{-s}g} \\
		&= \inn{(-\Delta)^{-s}f}{(-\Delta)^{-s}\sum_{j = 1}^{\infty} \int_K\phi_j g \dd \mu\phi_j} \\
		& =\inn{(-\Delta)^{-s}f}{\sum_{j = 1}^{\infty} \int_K\phi_j g \dd \mu \frac{1}{\lambda_{j}^{-s}}\phi_j} \\
		 &= \inn{f}{(-\Delta)^{-s} \sum_{j = 1}^{\infty} \int_K\phi_j g \dd \mu \frac{1}{\lambda_{j}^{-s}}\phi_j} \\
		 &= \inn{f}{ \sum_{j = 1}^{\infty} \int_K \phi_j g \dd \mu \frac{1}{\lambda_{j}^{-2s}}\phi_j} \\
...
We now quickly move to proving that the fractional Gaussian fields exist. First we will state the Bochner-Minlos theorem for nuclear spaces. This theorem is need for the proof of existence. It will be given without proof but can be found in \cite{minlos}.
...
In the following we want to prove that a FGF has an $L^2$ density \textcolor{red}{What is it?} if and only if $s > \frac{d_h}{2d_w}$. But for this we need to construct white noise. This will be done following \cite{LeGall}.

\textcolor{red}{This whole section}
Let $\mathcal{K}$ be the Borel $\sigma$-algebra on $K$ then since $\mu$ is a finite measure and therefore $\sigma$- finite. It then follows from \cite[Prop. 1.13]{LeGall} that there exists a probability field $(\Omega, \mathcal{F}, \pee)$ with a real-valued centered Gaussian random measure $W:L^{2}(\Omega, \mathcal{F}, \pee) \to \mathcal{K}$ with \textcolor{red}{density/intesity?} $\mu$.
