...
These simulations have had a varying rate of success. We have been able to simulate $X_s^{m}(x) = \sum\lambda_j \phi_j$ for a reasonable set of $s$-values, but we have encountered a limiting factor in the fact that we are not able to simulate $X_s^{m}$ on the Vicsek set for $m \geq 7$. The reason for this limitation lies in \Cref{discreteMatrix}. 
...
\section{The initial goal of the simulations}
...
\begin{figure}[!ht]
	...
	\end{minipage}
	\caption{Simulations of the Vicsek set on $V_3$ with different $s$ parameters.}
...
\begin{figure}[!ht]
	...
	\end{minipage}
	\caption{Simulations of the Sierpinski set on $V_3$ with different $s$ parameters.}
\end{figure}
