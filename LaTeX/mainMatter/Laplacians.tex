...
\begin{defi}\label{discreteLaplacian}
	Let $\ell(V_m)$ be the set of functions of $\func{f}{V_m}{\ree}$. Then for any $f \in \ell(V_m)$  the \emph{discrete Laplacian} on $V_m$ is defined as
	\begin{equation}
...
\begin{remark}\label{discreteMatrix}
	...
	\noindent Let $p_1, \ldots p_{N_m}$ be the vertices in $V_m$ then the \emph{Laplacian matrix} $L_m$ given by 
	\begin{equation}
	...
	\begin{equation}
		...
		\end{pmatrix}.
	\end{equation}
In addition $\#V_{m,p_i}$ is also called the \emph{degree of $p_i$}
\end{remark}

\section{Graph Energies}
In this section we will be following the arguments given by Strichartz in \cite[Chapter 1]{strichartz}. Many of the proofs are going to rely on the graph energy. Thus, we will first define and prove some properties of the graph energies form.
...
\noindent \textbf{Notation:} We will write $E_m(u) = E_m(u,u)$.
 ...
 Since we want to work on the whole Vicsek set $K$, we will want to create an extension of a function $u\colon V_m \to \ree$, such that it is defined on $V_{m+n}$. This is done in the following way:

\medskip
\begin{defi}
	Let $u\colon V_m \to \ree$. Then an \emph{extension of} $u$ is a function \nl $u'\colon V_{m+n} \to \ree$ where $u' \mid_{V_m} = u$.
\end{defi}
  One quickly notices that there must be a function that minimizes the graph energy of a given graph. We denote such a function with $\tilde{u}$ and call it a \emph{harmonic extension}.
...
\medskip
Given $V_0 = \cbr{q_1, q_2, q_3, q_4, q_5}$ and $u\colon V_0 \to \ree$ we have that
\begin{equation}
	E_0(u) = \left(u(q_1) - u(q_2)\right)^2 + \left(u(q_1) - u(q_3)\right)^2 + \left(u(q_1) - u(q_4)\right)^2 + \left(u(q_1) - u(q_5)\right)^2.
\end{equation} 
We are going to exploit the symmetry of the Vicsek set. Since all four "extremities" of the Vicsek set are identical we will only look at one of the limbs, since the other limbs will be the same computations. So if we want to minimize $E_1(u')$, for $u': V_1 to \ree$, we see for the extremity between $q_1$ and $q_2$ that
...
\end{figure}
Next we are going to optimize with regards to $u'(p_i)$ for $i = 0, 1, 2, 4$. Taking the derivative of the right side of \ref{E1Energy} and solving for $0$ we get the following
\begin{align}
...
We then get our renormalization constant for the Vicsek set to be $r = \frac{1}{3}$. Altogether we find that from \cref{renormalizedGraphEnergies} we have that
\begin{equation}
...
We are now ready to define the Kigami Laplacian. We will denote this with $\Delta_{\mu}$ as it depends on the choice of measure $\mu$. In our case we choose $\mu$ to be as in \Cref{normalziedHausdorffMeasure}.
...
\begin{defi}
	Given $m$ and a point $x \in V_m \backslash V_0$ then define the harmonic function \\ $\psi_{x}^{m}\colon V^{*} \to [0,1]$ that for a $y \in V_m$ is $\psi_x^{(m)}(y) = \delta_{xy}$.
\end{defi}
...
\begin{lemma}\label{itIsZero}
	Let $v\colon V^{*} \to \ree$ be a harmonic function on the Vicsek set. If $v(x) = v(y_i) = 0$ for all $y_i \in V_{m,x}$ then $v(\omega) = 0$ for all $\omega \in \overline{B_x}$ where $B_x = \left( \bigcup_{k > 0} S^{k}(x) \right) \cup \left(\bigcup_{i} \{y_i\} \right)$.
\end{lemma}
\begin{proof}
...
\begin{theorem} \label{pointwiseFormula}
	...
	\end{equation}
	holds with the limit uniform across $V_* \setminus V_0$. Conversely, suppose $u$ is a continuous function and the right side of \eqref{pointwiseEquation} converges uniformly to a continuous function on $V_* \setminus V_0$. Then $u \in \dom \Delta_{\mu}$ and the equation holds.
\end{theorem}
...
\begin{proof}
	...
	\end{equation}
	for any $x \in V_m \setminus V_0$. The right side needs to converge uniformly, so let $\epsilon > 0$. Since $f(x)$ is continuous then by \Cref{FmSmaller} we can choose $m$ large enough such that for all $x, y \in F_m$ we have that $\abs{f(x) - f(y)} < \epsilon$, in which case
	\begin{align}
	...
	\begin{align}
		\mathcal{E}_m(u,v) &= -\sum_{x \in V_m \setminus V_0} v(x) \left(\int_{K} \psi_{x}^{(m)} \dd\mu \right) r^{-m} \left(\int_{K} \psi_{x}^{(m)} \dd\mu \right)^{-1} \widetilde{\Delta}_{m}u(x) \\
		&= - \int_{K} \left( \sum_{x \in V_m \setminus V_0} v(x) f_m(x) \psi_x^{(m)}\right) \dd \mu. \\
	\end{align}
	...
	\begin{equation}\label{theVeryLongOne}
		\mathcal{E}(u, v) = \lim_{m \to \infty} \mathcal{E}_m(u,v) = \lim_{m \to \infty}-\int_{K} \left( \sum_{x \in V_m \setminus V_0} v(x) f_m(x) \psi_x^{(m)}\right) \dd \mu = - \int_{K} v(y)f(y) \dd \mu
	\end{equation}
	...
	\end{equation}
	First choose $m_3$ so $\abs{f_{m_3}(x)-f(x)} \leq \frac{\epsilon}{3 C_1}$. Next by continuity of $f$ choose $m_4$ so $\abs{f(x) -f(y)} \leq \frac{\epsilon}{3 C_1}$ for all $x \in F_{m_4}$. Lastly choose $m_5$ so \nl $\abs{f(y)-f_{m_5}(y)} \leq \frac{\epsilon}{3 C_1}$.
	...
	Next assume $y \in K \setminus V^{*}$, then since $V^{*}$ is dense in $K$ we can choose a sequence $(x_n)_{n = 1}$ where $x_n \to y$ as $n \to \infty$, and since $\psi_{x}^{(m)}$ is continuous on $K$ then $\psi_{x}^{(m)}(x_n) \to \psi_{x}^{(m)}(y)$ as $n \to \infty$. This gives us the following
	\begin{align}
...
\end{defi}
 ...
 \medskip
\begin{defi} A \emph{quadratic form} $\mathcal{E}$ on $\mathcal{H}$ is a non-negative, definite, symmetric bilinear form from $\dom \mathcal{E} \times \dom \mathcal{E} \to \ree$, where $\dom \mathcal{E}$ is a dense subspace of $\mathcal{H}$. 
...
\end{defi}
\begin{remark*}
	Note that $\frac{1}{a_m} \sum_{y \in V_m} \phi_{i}^{m}(y)f(y) = \inn{\phi^{m}_j}{f}_{L^{2}(V_m, \mu_m)}$ and therefore that \nl $(-\Delta_{m})^{-s}f(x) = \sum_{i = 1}^{N_m} (\lambda_{i}^{m})^{-s} \phi_{i}^{m}(x) \inn{\phi_j^{m}}{f}_{L^2(V_m, \mu_m)}$.
\end{remark*}

Having stated the discrete fractional Laplacian we move on to proving some of its useful properties.

\medskip
\begin{prop}\label{discreteFractionalLaplacianProperties}Let $(-\Delta_{m})^{-s}$ be defined as in \Cref{discreteFractionalLaplacian} then the follow holds
	\begin{enumerate}[(i)]
...
\begin{defi}\label{semigroupDef}
	Let $\mathcal{H}$ be a Hilbert space. A \emph{strongly continuous self-adjoint contraction semigroup} is a family of self-adjoint operators $(P_t)_{t \geq 0}\colon \mathcal{H} \to \mathcal{H}$
	\begin{enumerate}[(i)]
...
\noi \Cref{genSemigroup} is vital as it will show that an operator can generate a semigroup. 
...
\begin{proof}
	...
	Now showing that $\dom Q_{t}^{*} = \dom Q_t$. Since $Q_t$ is symmetric we have from the Cauchy-Schwartz inequality that
	\begin{equation}\label{operatorAndAdjointHaveSameDom}
		\abs{\inn{f}{Q_tg}} = \abs{\inn{Q_t f}{g}} \leq \norm{Q_t f} \norm{g}.
	\end{equation}
	Thus we have $Q_t$ is self-adjoint. 
	
	To show the semigroup properties from \Cref{semigroupDef}, we have (\ref{semigroup})
	\begin{equation}
	...
	\end{equation}
	and lastly (\ref{semigroup contraction})
	\begin{equation}
...
From \cite[Theorem 8.4]{barlow} this semigroup admits a heat kernel $p_t(x,y), t > 0, x,y \in K$, with respect to the normalized Hausdorff measure $\mu$. The heat kernel $p_t(x,y)$ is called the Dirichlet heat kernel on $K$. Further from \cite[section 2.3]{Fabrice} this heat kernel satisfies that for some $c_1, c_2 \in (0, \infty)$ 
...
\begin{defi}\label{fracLaplacian}
	Let $s \geq 0$, for $f \in L^2(K, \mu)$ the\emph{fractional Laplacian} $(-\Delta)^{-s}$ is defined as
	\begin{equation}
...
\begin{proof}
	...
	as desired.
\end{proof}
While the definition of the fractional Laplacian is not the most complicated, we are able to represent $(-\Delta)^{-}$ in another way, namely with the Riesz kernel.
...
\begin{prop}\label{boundsOfRieszKernel}
	Let $G_s(x,y)$ be defined as in \Cref{fractionalRieszKernel} then the following bounds hold
	\begin{enumerate}[(i)]
...
\begin{proof}
	...
	Now choosing $t = 1/4$ together with \eqref{heatKernelBound} that there is a constant $C > 0$ so that for every $x \in K$ we have
	\begin{equation}
	...
	\end{align}
	where the last inequality comes from the fact that $0 < \lambda_1 < \lambda_2 < \ldots$ and  \nl $t \geq 1$ so $e^{\lambda_j(1-t)} \leq e^{\lambda_1(1-t)}$. Thus we have the desired bound.
\end{proof}
...
\begin{proof}[Proof of \Cref{boundsOfRieszKernel}]
	...
	\noi Lastly for $s = d_h/d_w$, we again use the change of variable $t = ud(x,y)^{d_w}$ and setting $R = \diam(K)$ and noting that $0 < 1/R^{d_w} < 1/d(x,y)^{d_w}$, thus we find
	\begin{align}
		&\phantom{=}\int_{0}^{1} t^{s-1-\frac{d_h}{d_w}} \exp\left(-c_2 \left(\frac{d(x,y)^{d_w}}{t}\right)^{\frac{1}{d_w -1}}\right). \\
		...
		& \leq d_w \abs{\ln \left(d(x,y)\right)} +\int_{0}^{\frac{1}{R^{d_w}}} u^{-1} \exp\left(-c_2\left(\frac{1}{u}\right)^{\frac{1}{d_w+1}}\right) \dd u \\
...
\begin{defi} \label{NuclearSpace}
	Let $E$ be a locally convex Hausdorff space and $\norm{\cdot}_p$ a continuous seminorm on $E$. Further let $\widehat{E}_{p}$ be a Banach space, defined to be the completion of the normed space $E \setminus \ker \norm{\cdot}_p$. Then $E$ is a \emph{nuclear space} if to every continuous seminorm $\norm{\cdot}_p$ on $E$ there exists  another continuous seminorm on $E$ with $\norm{\cdot}_q \geq \norm{\cdot}_p$ such that the canonical mapping $\widehat{E}_{p} \to \hat{E}_q$ is \emph{nuclear}. 
...
\end{lemma}
\begin{proof}
	First, we will make sure the integral on the right side actually converges. This holds because $S(K) \subset C(K)$ whereby $f$ will be bounded on $K$. Thus we just need for every $x \in K$, that $\int_{K}G_s(x,y) \dd \mu(y)$ is convergent.\footnote{In \Cref{boundsOfRieszKernel}, (i) and (ii) the bounds are for $x \neq y$, but they are null-sets with respect to $\mu$}  For $s \in (d_h/d_w, \infty)$ the convergence follows from \cref{boundsOfRieszKernel}. Looking at $s \in (0, d_h/d_w]$, we will use a dyadic annuli decomposition. Meaning we will split $K$ up into infinitely many annuli on the form $ B(x, R2^{-j})\setminus B(x,RS^{-j-1})$, noting that
	\begin{equation}
		K \subseteq \bigcup_{j \geq 0} B(x, R2^{-j})\setminus B(x,RS^{-j-1})
	\end{equation}
	where $R = \diam(K)$. Then we have for all $y \in B(x, R2^{-j})\setminus B(x,RS^{-j-1})$ that
	\begin{equation}
		R2^{-j-1} \leq d(x,y) \leq R2^{-j-1} \Rightarrow 2^{-j-1} \leq R^{-1} \cd d(x,y) \leq 2^{-j-1} .
	\end{equation}
	  We will show that the integral is finite for both $s = d_h/d_w$ and $s<d_h/d_w$, but this requires we bound $\abs{\ln \left(d(x,y) \right)}$. So we see that for all $\alpha > 0$
	\begin{equation}
	...
	\end{align}
	Where we use \cref{normalziedHausdorffMeasureAhlforsRegular} and that $\int_{K}G_s(x,y)\dd \mu(y)$ is convergent for every $x \in K$.
...
\begin{coro}\label{coroForRieszKernelAndFractionalLaplacian} Let $(-\Delta)^{-s}$ be defined as in \Cref{fracLaplacian}, then the following holds
	\begin{enumerate}[(i)]
