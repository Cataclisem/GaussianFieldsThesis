...
\begin{proof}
	...
	\begin{multline}\label{theVeryLongOne}
		...
		= \lim_{m \to \infty}-\int_{K} \left( \sum_{x \in V_m \setminus V_0} v(x) f_m(x) \psi_x^{(m)}\right) \dd \mu = - \int_{K} v(y)f(y) \dd \mu \quad
	\end{multline}
...
\begin{defi}\label{selfAdjoint}
	...
	\begin{enumerate}[(i)]
		...
		\end{equation}
		and $c(f)$ denotes a constant that may depend on $f$. We can further define $A^*$ by the formula
		\begin{equation}
...
\end{defi}
\begin{remark*}
	Note that $\frac{1}{a_m} \sum_{y \in V_m} \phi_{i}^{m}(y)f(y) = \langle\phi^{m}_j,f \rangle_{L^{2}(V_m, \mu_m)}$ and therefore that \nl $(-\Delta_{m})^{-s}f(x) = \sum_{i = 1}^{N_m} (\lambda_{i}^{m})^{-s} \phi_{i}^{m}(x) \langle \phi_j^{m},f \rangle_{L^2(V_m, \mu_m)}$.
\end{remark*}
...
\end{defi}
\begin{remark*}
	Note that it follows from \eqref{heatKernelBound} that $G_s(x,y)$ converges for every $s \geq 0$ where $x \neq y$.
\end{remark*}
...
\begin{prop}\label{boundsOfRieszKernel}
	...
	\end{enumerate}
\end{prop}
Before proving \Cref{boundsOfRieszKernel} we will quickly state a lemma that is needed.
...
\begin{proof}[Proof of \Cref{boundsOfRieszKernel}]
	...
	\begin{equation}
		\int_{0}^{1} t^{s-1}p_t(x,y) \dd t \leq c_1 \int_{0}^{1} t^{s-1-\frac{d_h}{d_w}} \exp\left(-c_2 \left(\frac{d(x,y)^{d_w}}{t}\right)^{\frac{1}{d_w -1}}\right) \dd t.
	\end{equation}
	...
	\begin{equation}
		\int_{0}^{1} t^{s-1-\frac{d_h}{d_w}} \exp\left(-c_2 \left(\frac{d(x,y)^{d_w}}{t}\right)^{\frac{1}{d_w -1}}\right) \dd t \leq \int_{0}^{1}t^{s-1-\frac{d_h}{d_w}} \dd t.
	\end{equation}
	Next, if $s < d_h/d_w$, then using a change of variable $t = u d(x,y)^{d_w}$, we find that
	\begin{align}
		&\phantom{=}\int_{0}^{1} t^{s-1-\frac{d_h}{d_w}} \exp\left(-c_2 \left(\frac{d(x,y)^{d_w}}{t}\right)^{\frac{1}{d_w -1}}\right) \dd t \\
