...
\begin{defi}
	Given $m \geq 0$ and a point $x \in V_m \setminus V_0$ then define the \emph{harmonic function} \\ $\psi_{x}^{(m)}\colon V^{*} \to [0,1]$ that for a $y \in V_m$ is $\psi_x^{(m)}(y) = \delta_{xy}$.
\end{defi}
...
\medskip
\section{The Discrete Fractional Laplacian}
...
\section{The Fractional Laplacian}\label{sec:FractionalLaplacian}
